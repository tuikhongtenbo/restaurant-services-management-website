...
public interface ReservationService {
    ...
    // Lấy danh sách lịch đặt bàn phân trang (hỗ trợ lọc linh hoạt theo ngày và/hoặc
    // trạng thái)
    ...
    // Tiếp nhận thông tin đặt bàn mới, lưu ở trạng thái PENDING; capacity check
    // được thực hiện ở bước confirmReservation
    ReservationResponse createReservation(CreateReservationRequest request, UUID staffId);

    // Xác nhận đơn PENDING: kiểm tra capacity → CONFIRMED nếu còn chỗ, tự động
    // CANCELLED nếu hết chỗ
    ReservationResponse confirmReservation(UUID id, UUID tableId, UUID staffId);

    // Cập nhật thông tin khách hàng hoặc thời gian của một đơn đặt bàn (cho phép
    // sửa ở trạng thái PENDING/CONFIRMED)
    ReservationResponse update(UUID id, UpdateReservationRequest request);

    // Đánh dấu khách đã đến nhận bàn; đồng thời chuyển trạng thái bàn được gán sang
    // SERVING
    ...
    // Tác vụ tự động: quét và gán bàn trống tối ưu cho các đơn CONFIRMED sắp đến
    // giờ
    void autoAssignTables();

    // Tác vụ tự động: chuyển các đơn CONFIRMED quá giờ hẹn mà khách không check-in
    // sang NO_SHOW
    ...
    // Lấy thông tin tổng hợp đặt bàn trong ngày, bóc tách theo từng trạng thái
    // (dùng cho Dashboard/Calendar)
